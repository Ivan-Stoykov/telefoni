    ...
    async function fetchPhones() {
      try {
        ...
        if (response.ok) {
          ...
          if (initialPhoneId) {
            const selectedPhone = data.find(p => p.id === parseInt(initialPhoneId));
            if (selectedPhone) {
            ...
            {phone ? (
              ...
              <div className="d-flex flex-column h-100">
                ...
                <ul className="compare-specs-list">
                  <li><span>Dimensions:</span> {phone.phone_spec?.Dimensions || 'N/A'}</li>
                  <li><span>Weight:</span> {phone.phone_spec?.Weight || 'N/A'}</li>
                </ul>

                <h6 className="compare-category-title">Platform</h6>
                <ul className="compare-specs-list">
                  <li><span>OS:</span> {phone.phone_spec?.OS || 'N/A'}</li>
                  <li><span>Processor:</span> {phone.phone_spec?.processor ? `${phone.phone_spec.processor.brand} ${phone.phone_spec.processor.name}` : 'N/A'}</li>
                  <li><span>Battery:</span> {phone.phone_spec?.Battery || 'N/A'}</li>
                  <li><span>RAM:</span> {phone.RAM || 'N/A'}</li>
                  <li><span>Storage:</span> {phone.Storage || 'N/A'}</li>
                </ul>

                <h6 className="compare-category-title">Display And Audio</h6>
                <ul className="compare-specs-list">
                  <li><span>Type:</span> {phone.phone_spec?.ScreenType || 'N/A'}</li>
                  <li><span>Resolution:</span> {phone.phone_spec?.ScreenResolution || 'N/A'}</li>
                  <li><span>Size:</span> {phone.phone_spec?.ScreenSize || 'N/A'}</li>
                  <li><span>Protection:</span> {phone.phone_spec?.Protection || 'N/A'}</li>
                </ul>

                <h6 className="compare-category-title">Connectivity Features</h6>
                <ul className="compare-specs-list">
                  <li><span>WLAN:</span> {phone.phone_spec?.Wifi || 'N/A'}</li>
                  <li><span>Bluetooth:</span> {phone.phone_spec?.Bluetooth || 'N/A'}</li>
                  <li><span>NFC:</span> {phone.phone_spec?.NFC ? 'Yes' : 'No'}</li>
                  <li><span>Port:</span> {phone.phone_spec?.Port || 'N/A'}</li>
                </ul>
              ...
              <div className="empty-compare-slot position-relative h-100">
                <div className="mb-4">
                  <label className="text-muted small fw-bold mb-2">Compare with</label>
                  <input 
                    type="text" 
